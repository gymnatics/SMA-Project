class SimMap {
  constructor(nodes, connections) {
    ...
    this.rides = nodes.filter((node) => (node.type == "ride_a") || (node.type == "ride_b") || (node.type == "ride_c"));
    this.edges = [];
  ...
  getRideInfoFromNode(startNode) {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      ...
      if (i != startNodeIndex && ((this.nodes[i].type == "ride_a") || (this.nodes[i].type == "ride_b") || this.nodes[i].type == "ride_c")) {
  ...
  getAverageQueueTime_C(){
    let totalQueueC = 0;
    let numC = 0;
    for (let ride of this.rides){
      if (ride.type == "ride_c"){
        numC += 1;
        totalQueueC += ride.getQueueTime();
      }
    }
    return totalQueueC / numC;
  }
  getMinQueueTime() {
...
class MapNode {
  ...
  setTypePars() {
    ...
    if (this.type == "entrance") {
      ...
      this.img = null;
    } else if ((this.type == "ride_a") || (this.type == "ride_b") || (this.type == "ride_c")) {
      this.fill = "#0ab";
  ...
  getQueueTime() {
    if ((this.type == "ride_a") || (this.type == "ride_b") || (this.type == "ride_c")) {
