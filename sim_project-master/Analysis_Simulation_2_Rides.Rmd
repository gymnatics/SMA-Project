...
ggplot(sim_data2, aes(time)) + 
  ...
  geom_line(aes(y = time_in_queue),color = "darkred") + xlim(0,1500)
```

```{r}
ggplot(sim_data2, aes(time)) + 
  geom_line(aes(y = average_score),color = "orange") + xlim(0,1500)
```
...
```{r}
new_df <- filter(sim_data2, sim_data2$time > 3800)
