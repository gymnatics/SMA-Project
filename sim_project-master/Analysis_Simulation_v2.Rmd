...
# Introduction
The goal of our simulation is to analyze whether having 3 ride types or 2 ride types in the theme park is better. In order to do so, we will perform a paired t-test of the 2 different ride configurations while keeping the location of the rides the same.

In this output analysis, our group will aim to analyze the following parameters of our simulation:
...
Due to the time constraints, we chose to do do the Batch Means approach for data collection. This is because of 2 main reasons:
1. Our simulation is computationally expensive as it takes along time to run.
2. Our burn-in period is quite large, so the replication/deletion method would lead to alot of wasted data.
